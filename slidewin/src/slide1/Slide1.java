		...
		try {
			...
			for (int i = 1; i < 25; i++) {
				...
				while (!rs.isLast()) {// 直到读取到最后一条数据，跳出循环，运行下一条染色体
					...
					if (out == regionStart)// 判断下一个将移出窗口的snp位置out,是否恰好移出窗口
					{
						count--; // 移出窗口则计数减1
						po++; // 下一个
						rs.absolute(po); // 指针移到第pi个（下一个）将移出窗口的snp所在行
						out = rs.getInt("chromEnd"); // 获取下一个将移出窗口的snp位置out
					}
					...
					if (out == regionStart + 1) {
						d = 1;// 如果下一个将要移出窗口的snp位于滑窗最左端，下一次移动一位
					} else {
						d = out - regionStart - 1;// 否则使下一个将要移出窗口的snp，下一次移到位于滑窗最左端的位置
					}
					regionStart += d;
					regionEnd += d; // 窗口右滑
				}
